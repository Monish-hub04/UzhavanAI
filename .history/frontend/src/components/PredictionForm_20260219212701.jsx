...
export default function PredictionForm({ onSubmit, isLoading }) {
  ...
  const fillDemo = () => {
    ...
    reset(values);
    setLoadedProfile(name);
    setDemoIndex((prev) => (prev + 1) % demoProfiles.length);
  };

  const onInputChange = () => {
    if (loadedProfile) setLoadedProfile(null);
  };
    ...
    <section id="prediction" className="py-16 bg-gray-50/50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          ...
          {loadedProfile && (
            <div className="mt-6 flex items-center justify-center gap-2 text-xs font-medium text-primary-600 bg-primary-50/50 py-2 px-4 rounded-lg border border-primary-100 animate-fade-in">
              <Sparkles className="w-3 h-3" />
              Active Profile: <span className="font-bold">{loadedProfile}</span>
              <button
                onClick={() => setLoadedProfile(null)}
                className="ml-2 hover:text-primary-800 underline underline-offset-2"
              >
                Clear
              </button>
            </div>
